...
lst=[]
def simulate_revenue2(days_left, tickets_left, pricing_function, rev_to_date=0, demand_level_min=100,
                      demand_level_max=200, verbose=False):
    if (days_left == 0) or (tickets_left == 0):
        ...
        add = str("{:.0f} days before flight: "
                  "Started with {:.0f} seats. "
                  "Demand level: {:.0f}. "
                  "Price set to ${:.0f}. "
                  "Sold {:.0f} tickets. "
                  "Daily revenue is {:.0f}. Total revenue-to-date is {:.0f}. "
                  "{:.0f} seats remaining".format(days_left, tickets_left, demand_level, p, q, p * q,
                                                  p * q + rev_to_date, tickets_left - q))
        lst.append(add)

        return simulate_revenue2(days_left=days_left - 1,
...
def main():
    ...
    #menu = ['Parameters to input', 'Days left for the flight to take off', 'Tickets Left', 'Minimum Ticket Price']
    #choices = st.sidebar.checkbox("User Inputs", menu)


    #price_min = st.number_input('Enter a number to maintain a minimum price for the tickets sold')
    ...
    if st.button("RUN"):
        ...
        def score_me(pricing_function, sims_per_scenario=5):
            ...
            scenario_score = [simulate_revenue2(s.n_days, s.n_tickets, pricing_function)
                                 for _ in range(sims_per_scenario)]
            n=np.argmax(scenario_score)
            m=np.argmin(scenario_score)
            max_val= max(scenario_score)
            min_val=min(scenario_score)

            st.header("Finding demand levels combination for highest revenue")
            mxi=(n*30)
            mxf=(n+1)*30
            max_store=lst[mxi:mxf]
            for i in max_store:
                st.write(i)
            ...
            st.subheader("Ran {:.0f} flights starting {:.0f} days before flight with {:.0f} tickets. "
                  "Max revenue: ${:.0f}".format(sims_per_scenario,
                                                    s.n_days,
                                                    s.n_tickets,
                                                    max_val))
            st.header("Finding demand levels combination for least revenue")
            mii = (m * 30)
            mif = (m+1) * 30
            min_store=lst[mii:mif]
            for i in min_store:
                st.write(i)
            st.subheader("Ran {:.0f} flights starting {:.0f} days before flight with {:.0f} tickets. "
                  "Min revenue: ${:.0f}".format(sims_per_scenario,
                                                    s.n_days,
                                                    s.n_tickets,
                                                    min_val))
